...
csVector::csVector (int ilimit, int ithreshold)
{
  limit = ilimit;
  if (ilimit)
    root = (csSome *)malloc (ilimit * sizeof (csSome));
  else
    root = NULL;
...
bool csVector::Insert (int n, csSome Item)
{
  if (n <= count)
  {
    ...
    uint movebytes = (count - n - 1) * sizeof (csSome);
    if (movebytes)
...
bool csVector::Shift (int n1, int n2)
{
  ...
  csSome x = root [n1];
  if (n1 < n2)
    memmove (&root [n1], &root [n1 + 1], (n2 - n1) * sizeof (csSome));
  else
    memmove (&root [n2 + 1], &root [n2], (n1 - n2) * sizeof (csSome));
...
int csVector::Find (csSome which, bool reversedir) const
{
  int i;
...
bool csVector::Copy (csVector &Other)
{
  ...
  memcpy (root, Other.root, count * sizeof (csSome));
  return true;
}
